...
import {schema} from './benchmark/schema.ts';

/**
 * HOW TO run the benchmarks:
 *
 * sudo docker-compose up -d postgres
 * sudo docker-compose run benchmark_server node test/benchmark/seed.ts
 ...
 */
export function startBenchmarkServer(port = 4001): Server {
 ...
 */
function formatBenchmarkError(error: Readonly<Error>) {
...
const graphqlHandler = createHandler({
  schema,
  // graphql-http accepts the serialized error shape at runtime, while its
  // declaration currently restricts this hook to Error instances.
  formatError: formatBenchmarkError as unknown as NonNullable<
    Parameters<typeof createHandler>[0]['formatError']
  >
});
 ...
 * @return {void}
 */
function handleBenchmarkRequest(
