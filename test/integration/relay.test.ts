...
async function generateCustom(id: string | number) {
  return {
    id,
...
describe('relay', function () {
  ...
  type RelayModel = ReturnType<typeof sequelize.define>;
  type RelayInstance = Awaited<ReturnType<RelayModel['create']>>;
  const state = {} as {
    project: RelayInstance;
    userA: RelayInstance;
    userB: RelayInstance;
    users: RelayInstance[];
  };
  ...
  let nodeInterface: ReturnType<typeof sequelizeNodeInterface>['nodeInterface'];
  let Project;
  let projectType: GraphQLObjectType;
  let viewerType: GraphQLObjectType;
  let nodeField: ReturnType<typeof sequelizeNodeInterface>['nodeField'];
  ...
  beforeAll(() => {
    ...
    // These specs query by primary key (user(id: N)). `id` is generated by
    // sequelize rather than declared by the fixture, so it must be opted in
    // or argsToFindOptions drops the argument and an arbitrary row comes back.
  ...
  beforeAll(() => {
    return state.project.setUsers([state.userA.id, state.userB.id]);
  });

  it('should support unassociated GraphQL types', () => {
    const globalId = toGlobalId('Viewer', 1);
  ...
  describe('node queries', () => {
    it('should allow returning a custom entity', async () => {
      const custom = await generateCustom(1);
