...
import type { SimplifiedASTCollection } from '../../src/simplifyAST.js';
 ...
 */
function parseFirstDefinition(query: string): DefinitionNode {
...
describe('simplifyAST', function () {
  ...
  it('should simplify a variable-backed field argument from resolver info', async function () {
    let simplified: SimplifiedASTCollection | undefined;
  ...
  it('should ignore inherited variable map entries', async function () {
    ...
    expect(result.errors).to.equal(undefined);
    if (!simplified?.fields) {
      throw new TypeError('Expected the resolver to simplify its AST.');
    }
  ...
  it('should distinguish a field from a fragment with the same name', async function () {
    ...
    expect(data?.viewer.viewer).to.equal('ok');
    expect(simplified).to.deep.equal({
      fields: {
        viewer: {
          args: {},
  ...
  it('should expose a $parent', function () {
    const ast = simplifyAST(parseFirstDefinition(`
      {
        users {
          name
          projects(first: 1) {
